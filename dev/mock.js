...
function mockFactory(conf) {
  ...
  function mock(req, res, next) {
    ...
    res.mock(filePath, shouldSkipNotFound)
  }
...
function mockMethod (req, res, next) {
  ...
  function mock(filePath, shouldSkipNotFound) {
    ...
    if (ext != "") {
      return res.sendFile(filePath, function(err) {
        if (err) {
          ...
          mockRESTful(filePath, shouldSkipNotFound)
        }
      })
    }
    mockRESTful(filePath, shouldSkipNotFound)
  ...
  function mockRESTful(filePath, shouldSkipNotFound){
